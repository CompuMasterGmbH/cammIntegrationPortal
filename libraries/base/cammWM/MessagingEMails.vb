...
#Region "SendEMail"
        ...
        Public Function SendEMail(ByVal rcptName As String, ByVal rcptAddress As String, ByVal msgSubject As String, ByVal msgTextBody As String, ByVal msgHtmlBody As String, ByVal senderName As String, ByVal senderAddress As String, ByVal attachments As CompuMaster.camm.WebManager.Messaging.EMailAttachment(), Optional ByVal priority As CompuMaster.camm.WebManager.Messaging.EMails.Priority = Nothing, Optional ByVal sensitivity As CompuMaster.camm.WebManager.Messaging.EMails.Sensitivity = Nothing, Optional ByVal requestTransmissionConfirmation As Boolean = False, Optional ByVal requestReadingConfirmation As Boolean = False, Optional ByVal additionalHeaders As Collections.Specialized.NameValueCollection = Nothing, Optional ByVal msgCharset As String = Nothing, Optional ByRef bufErrorDetails As String = Nothing) As Boolean
            ...
            ElseIf _WebManager.IsSupported.Messaging.eMail Then
                ...
                If _MailSendingSystem = CompuMaster.camm.WebManager.Messaging.EMails.MailSendingSystem.Auto Then
                    _MailSendingSystem = CompuMaster.camm.WebManager.Messaging.EMails.MailSendingSystem.EasyMail
                ...
                If _MailSendingSystem = CompuMaster.camm.WebManager.Messaging.EMails.MailSendingSystem.EasyMail Then
                    Try
                        Result = System_SendEMailEx_EasyMail(rcptName, rcptAddress, msgSubject, msgTextBody, msgHtmlBody, senderName, senderAddress, msgCharset, bufErrorDetails, attachments, priority, sensitivity, requestTransmissionConfirmation, requestReadingConfirmation, additionalHeaders)
                    Catch ex As Exception
                        Me._WebManager.Log.RuntimeException(ex, True, False, WMSystem.DebugLevels.Medium_LoggingOfDebugInformation) '"Error while sending an email with EasyMail! Perhaps the smtp.Net.dll is missing!" & vbNewLine & ex.Message, ex.StackTrace, WMSystem.DebugLevels.NoDebug, Fal)
                        'e.g. smtp.Net.dll from EasyMail is missing in the bin-folder
                        'delegate to standard .net mailssystem
                        _MailSendingSystem = MailSendingSystem.NetFramework
                    ...
                    End Try
                ElseIf _MailSendingSystem = CompuMaster.camm.WebManager.Messaging.EMails.MailSendingSystem.NetFramework Then
                    Result = System_SendEMailEx_NetFramework(rcptName, rcptAddress, msgSubject, msgTextBody, msgHtmlBody, senderName, senderAddress, msgCharset, bufErrorDetails, attachments, priority, sensitivity, requestTransmissionConfirmation, requestReadingConfirmation, additionalHeaders)
                Else
        ...
        Friend Function SendEMail(ByVal allowQueuing As Boolean, ByVal rcptAddresses_To As String, ByVal rcptAddresses_Cc As String, ByVal rcptAddresses_Bcc As String, ByVal MsgSubject As String, ByVal MsgTextBody As String, ByVal MsgHTMLBody As String, ByVal SenderName As String, ByVal SenderAddress As String, ByVal attachments As Messaging.EMailAttachment(), Optional ByVal priority As Messaging.EMails.Priority = Nothing, Optional ByVal sensitivity As Messaging.EMails.Sensitivity = Nothing, Optional ByVal requestTransmissionConfirmation As Boolean = False, Optional ByVal requestReadingConfirmation As Boolean = False, Optional ByVal additionalHeaders As Collections.Specialized.NameValueCollection = Nothing, Optional ByVal msgCharset As String = Nothing, Optional ByRef bufErrorDetails As String = Nothing) As Boolean
            ...
            ElseIf _WebManager.IsSupported.Messaging.eMail Then
                ...
                If _MailSendingSystem = CompuMaster.camm.WebManager.Messaging.EMails.MailSendingSystem.EasyMail Then
                    Try
                        Result = System_SendEMail_MultipleRcpts_EasyMail(rcptAddresses_To, rcptAddresses_Cc, rcptAddresses_Bcc, MsgSubject, MsgTextBody, MsgHTMLBody, SenderName, SenderAddress, msgCharset, bufErrorDetails, attachments, priority, sensitivity, requestTransmissionConfirmation, requestReadingConfirmation, additionalHeaders)
                    Catch ex As Exception
                        Me._WebManager.Log.RuntimeException(ex, True, False, WMSystem.DebugLevels.Medium_LoggingOfDebugInformation) '"Error while sending an email with EasyMail! Perhaps the smtp.Net.dll is missing!" & vbNewLine & ex.Message, ex.StackTrace, WMSystem.DebugLevels.NoDebug, Fal)
                        'e.g. smtp.Net.dll from EasyMail is missing in the bin-folder
                        'or Comparing the assembly name resulted in the mismatch: Revision Number
                        'delegate to standard .net mailssystem
                        _MailSendingSystem = MailSendingSystem.NetFramework
...
#Region "Utils"
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Shared Function SplitEMailAddressIntoAddressParts(ByVal emailAddressInSmtpFormat As String) As EMailReceipient()
            Dim Result As String() = Utils.SplitString(emailAddressInSmtpFormat, ","c, "\"c)
            Dim EmailReceipient(Result.Length - 1) As EMailReceipient
            For MyCounter As Integer = 0 To Result.Length - 1
                Dim Receipient As New EMailReceipient

                If Not Result(MyCounter).LastIndexOf("<") = -1 Then
                    Receipient.Address = Result(MyCounter).Substring(Result(MyCounter).LastIndexOf("<"), Result(MyCounter).Length - Result(MyCounter).LastIndexOf("<"))
                    Receipient.Address = Receipient.Address.Replace("<", "").Replace(">", "")
                Else
                    Receipient.Address = Result(MyCounter)
                End If

                If Result(MyCounter).LastIndexOf(" ") > 0 AndAlso Result(MyCounter).LastIndexOf(" ") = Result(MyCounter).LastIndexOf("<") - 1 Then
                    Receipient.Name = Result(MyCounter).Substring(0, Result(MyCounter).LastIndexOf(" "))
                    Receipient.Name = Receipient.Name.Replace("<", "").Replace(">", "")
                Else
                    Receipient.Name = String.Empty
                End If

                EmailReceipient(MyCounter) = Receipient
            Next
...
#Region "EasyMail .Net Edition"
        ''' -----------------------------------------------------------------------------
        ''' <summary>
        ''' Send an email via EasyMail
        ''' !!! Pay attention: Update changes in System_SendEMail_MultipleRcpts_EasyMail, too !!!
        ''' </summary>
        ''' <param name="RcptName"></param>
        ''' <param name="RcptAddress"></param>
        ''' <param name="MsgSubject"></param>
        ''' <param name="MsgTextBody"></param>
        ''' <param name="MsgHTMLBody"></param>
        ''' <param name="SenderName"></param>
        ''' <param name="SenderAddress"></param>
        ''' <param name="MsgCharset"></param>
        ''' <param name="bufErrorDetails"></param>
        ''' <param name="Attachments"></param>
        ''' <param name="Priority"></param>
        ''' <param name="Sensitivity"></param>
        ''' <param name="RequestTransmissionConfirmation"></param>
        ''' <param name="RequestReadingConfirmation"></param>
        ''' <param name="AdditionalHeaders"></param>
        ''' <returns></returns>
        ''' <remarks>
        ''' </remarks>
        ''' <history>
        ''' 	[zeutzheim]	07.12.2007	Created
        ''' </history>
        ''' -----------------------------------------------------------------------------
        Private Function System_SendEMailEx_EasyMail(ByVal RcptName As String, ByVal RcptAddress As String, ByVal MsgSubject As String, ByVal MsgTextBody As String, ByVal MsgHTMLBody As String, ByVal SenderName As String, ByVal SenderAddress As String, ByVal MsgCharset As String, ByRef bufErrorDetails As String, ByVal Attachments As EMailAttachment(), ByVal Priority As CompuMaster.camm.WebManager.Messaging.EMails.Priority, ByVal Sensitivity As CompuMaster.camm.WebManager.Messaging.EMails.Sensitivity, ByVal RequestTransmissionConfirmation As Boolean, ByVal RequestReadingConfirmation As Boolean, ByVal AdditionalHeaders As Collections.Specialized.NameValueCollection) As Boolean
            ...
            Dim SMTPServer As New Quiksoft.EasyMail.SMTP.SMTPServer
            Dim smtpObj As New Quiksoft.EasyMail.SMTP.SMTP
            Dim msgBody As New Quiksoft.EasyMail.SMTP.BodyPart
            Dim Result As Boolean = True 'False, if there is an error

            'Configure SMTP-Server-Settings
            Try
                SMTPServer.Name = Me._WebManager.SMTPServerName
                SMTPServer.Port = Me._WebManager.SMTPServerPort
                Select Case UCase(_WebManager.SMTPAuthType)
                    Case "LOGIN"
                        ...
                        SMTPServer.AuthMode = Quiksoft.EasyMail.SMTP.SMTPAuthMode.AuthLogin
                        SMTPServer.Account = Me._WebManager.SMTPUserName
                        SMTPServer.Password = Me._WebManager.SMTPPassword
                    Case "PLAIN"
                        If Me._WebManager.SMTPUserName = Nothing Then Throw New ArgumentNullException("WebManager.SmtpUserName")
                        If Me._WebManager.SMTPPassword = Nothing Then Throw New ArgumentNullException("WebManager.SmtpPassword")
                        SMTPServer.AuthMode = Quiksoft.EasyMail.SMTP.SMTPAuthMode.AuthLogin
                        ...
                        SMTPServer.Password = Me._WebManager.SMTPPassword
                    Case "CRAM-MD5"
                        SMTPServer.AuthMode = Quiksoft.EasyMail.SMTP.SMTPAuthMode.CramMD5
                        SMTPServer.Account = Me._WebManager.SMTPUserName
                        SMTPServer.Password = Me._WebManager.SMTPPassword
                    Case Else '==>NONE
                        SMTPServer.AuthMode = Quiksoft.EasyMail.SMTP.SMTPAuthMode.None
                        SMTPServer.Account = Me._WebManager.SMTPUserName
                        SMTPServer.Password = Me._WebManager.SMTPPassword
                End Select


                'Add the SMTP-Server to the SMTP-Object, which is responsible for sending the mail
                smtpObj.SMTPServers.Add(SMTPServer)

                'Add a normal recipient
                eMail.Recipients.Add(RcptAddress, RcptName, Quiksoft.EasyMail.SMTP.RecipientType.To)

                'Specify the sender
                eMail.From.Email = SenderAddress
                ...
                If Not Priority = Nothing Then
                    ...
                    End Select
                    eMail.Priority = CType(Priority, Integer).ToString
                    If Priority = CompuMaster.camm.WebManager.Messaging.EMails.Priority.High OrElse Priority = CompuMaster.camm.WebManager.WMSystem.MailImportance.Normal OrElse Priority = CompuMaster.camm.WebManager.Messaging.EMails.Priority.Low Then
                        eMail.CustomHeaders.Add("X-MSMail-Priority", Priority.ToString())
                        eMail.CustomHeaders.Add("Importance", Priority.ToString())
                    End If
                End If
                ...
                If Not Sensitivity = Nothing Then
                    Select Case Sensitivity
                        Case Sensitivity.Status_CompanyConfidential
                            eMail.CustomHeaders.Add("Sensitivity", "Company-Confidential")
                        Case Sensitivity.Status_Personal
                            eMail.CustomHeaders.Add("Sensitivity", "Personal")
                        Case Sensitivity.Status_Private
                            eMail.CustomHeaders.Add("Sensitivity", "Private")
                    End Select
                End If

                'Setup mail receipt confirmations
                If RequestReadingConfirmation Then
                    eMail.DispositionNotification.To.Email = SenderAddress
                    eMail.DispositionNotification.To.Name = SenderName
                End If

                If RequestTransmissionConfirmation Then
                    'Doesn't work with some E-Mail-Clients
                    'e.g. Outlook does not support this header
                    eMail.ReturnReceiptTo = """" & SenderName & """ <" & SenderAddress & ">"
                End If

                'Setup additional headers
                If Not AdditionalHeaders Is Nothing Then
                    For Each MyAdditionalHeader As String In AdditionalHeaders
                        eMail.CustomHeaders.Add(MyAdditionalHeader, AdditionalHeaders(MyAdditionalHeader))
                    Next
                End If

                'Add attachments/ related/ embedded objects
                If Not Attachments Is Nothing Then
                    For Each MyAttachment As EMailAttachment In Attachments
                        If MyAttachment.PlaceholderInMhtmlToBeReplacedByContentID = "" Then
                            'Standard Attachment
                            If MyAttachment.FilePath <> "" Then
                                eMail.Attachments.Add(MyAttachment.FilePath)
                            ElseIf Not MyAttachment.RawData Is Nothing AndAlso MyAttachment.RawDataFilename <> "" AndAlso MyAttachment.RawData.Length > 0 Then
                                Dim attachment As New Quiksoft.EasyMail.SMTP.Attachment
                                Dim MyAttachmentStream As System.IO.Stream = New System.IO.MemoryStream(MyAttachment.RawData)
                                eMail.Attachments.Add(MyAttachmentStream, MyAttachment.RawDataFilename)
                            Else
                                _WebManager.Log.RuntimeException("Attachment information partially missing")
                            End If
                        Else
                            'Standard Attachment
                            ...
                            Dim attachment As New Quiksoft.EasyMail.SMTP.Attachment
                            attachment.MimeFlags = MIMEHeaderFlags.NoDisposition
                            ...
                            MsgHTMLBody = Utils.ReplaceString(MsgHTMLBody, MyAttachment.PlaceholderInMhtmlToBeReplacedByContentID, newContentIDName, Utils.ReplaceComparisonTypes.InvariantCultureIgnoreCase)
                            'Replace url-encoded matches
                            MsgHTMLBody = Utils.ReplaceString(MsgHTMLBody, System.Web.HttpUtility.UrlEncode(MyAttachment.PlaceholderInMhtmlToBeReplacedByContentID), newContentIDName, Utils.ReplaceComparisonTypes.InvariantCultureIgnoreCase)
                            'Replace html- and url-encoded matches
                            MsgHTMLBody = Utils.ReplaceString(MsgHTMLBody, System.Web.HttpUtility.HtmlEncode(System.Web.HttpUtility.UrlEncode(MyAttachment.PlaceholderInMhtmlToBeReplacedByContentID)), newContentIDName, Utils.ReplaceComparisonTypes.InvariantCultureIgnoreCase)
                            MyAttachment.PlaceholderInMhtmlToBeReplacedByContentID = newContentIDName
                            attachment.CustomHeaders.Add("Content-ID", "<" & (MyAttachment.PlaceholderInMhtmlToBeReplacedByContentID) & ">")

                            'Add attachment data
                            If MyAttachment.FilePath <> "" Then
                                attachment.Filename = MyAttachment.FilePath
                                attachment.DisplayName = System.IO.Path.GetFileName(MyAttachment.FilePath)
                                attachment.CustomHeaders.Add("Content-Disposition", "inline; filename=""" & System.IO.Path.GetFileName(MyAttachment.FilePath) & """")
                            ElseIf Not MyAttachment.RawData Is Nothing AndAlso MyAttachment.RawDataFilename <> "" AndAlso MyAttachment.RawData.Length > 0 Then
                                Dim MyAttachmentStream As System.IO.Stream = New System.IO.MemoryStream(MyAttachment.RawData)
                                attachment.Stream = MyAttachmentStream
                                attachment.DisplayName = MyAttachment.RawDataFilename
                                attachment.CustomHeaders.Add("Content-Disposition", "inline; filename=""" & MyAttachment.RawDataFilename & """; size=" & MyAttachment.RawData.Length & "")
                            Else
                                _WebManager.Log.RuntimeException("Attachment information partially missing")
                ...
                'Set message body
                If Not MsgTextBody Is Nothing And Not MsgTextBody = "" Then
                    eMail.BodyParts.Add(MsgTextBody, Quiksoft.EasyMail.SMTP.BodyPartFormat.Plain)
                End If
                If Not MsgHTMLBody Is Nothing And Not MsgHTMLBody = "" Then
                    eMail.BodyParts.Add(MsgHTMLBody, Quiksoft.EasyMail.SMTP.BodyPartFormat.HTML)
                End If

                'Set charset
                If MsgCharset = "" Then
                    eMail.CharsetEncoding = Text.UTF8Encoding.UTF8
                Else
                    Try
                        eMail.CharsetEncoding = Text.Encoding.GetEncoding(MsgCharset)
                    Catch
                        eMail.CharsetEncoding = Text.UTF8Encoding.UTF8
                    End Try
                End If
                ...
                Catch LicenseExcep As LicenseException
                    Me._WebManager.Log.RuntimeException("License key error: " + LicenseExcep.ToString)
                    bufErrorDetails = "License key error: " + LicenseExcep.ToString
                    Return False
                Catch FileIOExcep As FileIOException
                    Me._WebManager.Log.RuntimeException("File IO error: " + FileIOExcep.ToString)
                    bufErrorDetails = "File IO error: " + FileIOExcep.ToString
                    Return False
                Catch SMTPAuthExcep As SMTPAuthenticationException
                    Me._WebManager.Log.RuntimeException("SMTP Authentication error: " + SMTPAuthExcep.ToString)
                    bufErrorDetails = "SMTP Authentication error: " + SMTPAuthExcep.ToString
                    Return False
                Catch SMTPConnectExcep As SMTPConnectionException
                    Me._WebManager.Log.RuntimeException("Connection error: " + SMTPConnectExcep.ToString)
                    bufErrorDetails = "Connection error: " + SMTPConnectExcep.ToString
                    Return False
                Catch SMTPProtocolExcep As SMTPProtocolException
                    Me._WebManager.Log.RuntimeException("SMTP protocol error: " + SMTPProtocolExcep.ToString)
                    bufErrorDetails = "SMTP protocol error: " + SMTPProtocolExcep.ToString
                    Return False
                Catch ex As Exception
                    Me._WebManager.Log.RuntimeException("Error: " + ex.ToString)
                    bufErrorDetails = "Error: " + ex.ToString
                    Return False
                End Try
...
#If Not DEBUG Then
                bufErrorDetails = ex.ToString
#Else
                If _WebManager.DebugLevel >= WMSystem.DebugLevels.Medium_LoggingOfDebugInformation Then
                    bufErrorDetails = ex.ToString
                Else
...
#End If
        ...
        ''' -----------------------------------------------------------------------------
        ''' <summary>
        ''' Send an email to multiple recepients via EasyMail
        ''' !!! Pay attention: Update changes in System_SendEMailEx_EasyMail, too !!!
        ''' </summary>
        ''' <param name="RcptAddresses_To"></param>
        ''' <param name="RcptAddresses_CC"></param>
        ''' <param name="RcptAddresses_BCC"></param>
        ''' <param name="MsgSubject"></param>
        ''' <param name="MsgTextBody"></param>
        ''' <param name="MsgHTMLBody"></param>
        ''' <param name="SenderName"></param>
        ...
        Private Function System_SendEMail_MultipleRcpts_EasyMail(ByVal RcptAddresses_To As String, ByVal RcptAddresses_CC As String, ByVal RcptAddresses_BCC As String, ByVal MsgSubject As String, ByVal MsgTextBody As String, ByVal MsgHTMLBody As String, ByVal SenderName As String, ByVal SenderAddress As String, Optional ByVal MsgCharset As String = Nothing, Optional ByRef bufErrorDetails As String = Nothing, Optional ByVal Attachments As CompuMaster.camm.WebManager.Messaging.EMailAttachment() = Nothing, Optional ByVal Priority As CompuMaster.camm.WebManager.Messaging.EMails.Priority = Nothing, Optional ByVal Sensitivity As CompuMaster.camm.WebManager.Messaging.EMails.Sensitivity = Nothing, Optional ByVal RequestTransmissionConfirmation As Boolean = False, Optional ByVal RequestReadingConfirmation As Boolean = False, Optional ByVal AdditionalHeaders As Collections.Specialized.NameValueCollection = Nothing) As Boolean
            ...
            Try
                ...
                'Setup transmission confirmation
                If RequestTransmissionConfirmation Then
                    eMail.ReturnReceiptTo = """" & SenderName & """ <" & SenderAddress & ">"
                End If
                ...
                If Not Attachments Is Nothing Then
                    For Each MyAttachment As EMailAttachment In Attachments
                        If MyAttachment.PlaceholderInMhtmlToBeReplacedByContentID = "" Then
                            'Standard Attachment
                            If Not MyAttachment.FilePath Is Nothing AndAlso MyAttachment.FilePath <> "" Then
                                Dim fi As New System.IO.FileInfo(MyAttachment.FilePath)
                                If fi.Exists Then
                                    eMail.Attachments.Add(MyAttachment.FilePath)
                                Else
                                    _WebManager.Log.RuntimeException("Attachment not found: " & MyAttachment.FilePath)
                                    bufErrorDetails = "Attachment not found: " & MyAttachment.FilePath
                                    Return False
                                End If
                            ElseIf Not MyAttachment.RawData Is Nothing AndAlso MyAttachment.RawDataFilename <> "" AndAlso MyAttachment.RawData.Length > 0 Then
                                Dim attachment As New Quiksoft.EasyMail.SMTP.Attachment
                        ...
                        Else
                            Try
                                'Try to add an embedded attachment
                                'Fails when the add method cannot replace the placeholder with its contentID
                                'So we catch the exception and add the "embedded attachment" as a standard attachment
                                'We have to handle 2 ways of embedded attachments!!!
                                'In case of MailSendingSystem queue, the attachment is included in MyAttachment.RawData
                                'In case of MailSendingSystem EasyMail, the attachment is included in MyAttachment.FilePath or MyAttachment.RawData (it belongs to the way the attachment is initialized)
                                If Not MyAttachment.FilePath Is Nothing AndAlso MyAttachment.FilePath <> "" Then
                                    eMail.BodyPartItems.Add(MyAttachment.FilePath, MyAttachment.PlaceholderInMhtmlToBeReplacedByContentID)
                                ElseIf Not MyAttachment.RawData Is Nothing Then
                                    Dim MyAttachmentStream As System.IO.Stream = New System.IO.MemoryStream(MyAttachment.RawData)
                                    eMail.BodyPartItems.Add(MyAttachmentStream, MyAttachment.PlaceholderInMhtmlToBeReplacedByContentID)
                                Else
                                    _WebManager.Log.RuntimeException("Attachment information partially missing")
                                End If
                            Catch
                ...
                'Receipients and message
                If _WebManager.System_DebugLevel >= WMSystem.DebugLevels.Medium_LoggingAndRedirectAllEMailsToDeveloper Then
                    eMail.Recipients.Add(_WebManager.DevelopmentEMailAccountAddress, _WebManager.DevelopmentEMailAccountAddress)
                    If MsgTextBody <> "" Then 'text body present
                        msgBody.Body = "The server currently runs in debug mode. All e-mails will be sent to your address only." & ControlChars.CrLf & ControlChars.CrLf & ControlChars.CrLf & MsgTextBody
                        If MsgHTMLBody <> "" Then 'additional HTML body
                            msgBody.Body = "<h4><font face=""Arial"" color=""red""><b><em>The server currently runs in debug mode. All e-mails will be sent to your address only.</em></b></font></h4>" & MsgHTMLBody
                        End If
                    ElseIf MsgHTMLBody <> "" Then 'without text body; HTML only
                        msgBody.Body = "<h4><font face=""Arial"" color=""red""><b><em>The server currently runs in debug mode. All e-mails will be sent to your address only.</em></b></font></h4>" & MsgHTMLBody
                    End If
                Else

                    If RcptAddresses_To <> "" Then
                        'Never use the Parse-Method!!! Use Add-Method instead
                        Dim receipients As EMailReceipient() = EMails.SplitEMailAddressIntoAddressParts(RcptAddresses_To)
                        For myCounter As Integer = 0 To receipients.Length - 1
                            eMail.Recipients.Add(receipients(myCounter).Address, receipients(myCounter).Name, RecipientType.To)
                        Next
                    End If

                    If RcptAddresses_CC <> "" Then
                        Dim receipients As EMailReceipient() = EMails.SplitEMailAddressIntoAddressParts(RcptAddresses_CC)
                        For myCounter As Integer = 0 To receipients.Length - 1
                            eMail.Recipients.Add(receipients(myCounter).Address, receipients(myCounter).Name, RecipientType.CC)
                        Next
                    End If

                    If RcptAddresses_BCC <> "" Then
                        Dim receipients As EMailReceipient() = EMails.SplitEMailAddressIntoAddressParts(RcptAddresses_BCC)
                        For myCounter As Integer = 0 To receipients.Length - 1
                            eMail.Recipients.Add(receipients(myCounter).Address, receipients(myCounter).Name, RecipientType.BCC)
                        Next
                    End If

                    If MsgTextBody <> "" Then 'text body present
                        msgBody.Body = MsgTextBody
                        If MsgHTMLBody <> "" Then 'additional HTML body
                            msgBody.Body = MsgHTMLBody
                        End If
                    ElseIf MsgHTMLBody <> "" Then
                        msgBody.Body = MsgHTMLBody
                    End If
                End If
        ...
        Private Function SendViaQueue(ByVal rcptAddresses_To As String, ByVal rcptAddresses_CC As String, ByVal rcptAddresses_BCC As String, ByVal msgSubject As String, ByVal msgTextBody As String, ByVal msgHTMLBody As String, ByVal senderName As String, ByVal senderAddress As String, ByVal msgCharset As String, ByVal connection As IDbConnection, ByRef bufErrorDetails As String, ByVal attachments() As Messaging.EMailAttachment, ByVal priority As CompuMaster.camm.WebManager.Messaging.EMails.Priority, ByVal sensitivity As CompuMaster.camm.WebManager.Messaging.EMails.Sensitivity, ByVal requestTransmissionConfirmation As Boolean, ByVal requestReadingConfirmation As Boolean, ByVal additionalHeaders As Collections.Specialized.NameValueCollection) As Boolean
            ...
            Try
                ...
                If _WebManager.System_DebugLevel >= WMSystem.DebugLevels.Medium_LoggingAndRedirectAllEMailsToDeveloper Then
                    ...
                    End If
                    If msgHTMLBody <> "" Then 'without text body; HTML only
...
#Region ".NET Framework standard methods"
        ...
        Private Function System_SendEMail_MultipleRcpts_NetFramework(ByVal RcptAddresses_To As String, ByVal RcptAddresses_CC As String, ByVal RcptAddresses_BCC As String, ByVal MsgSubject As String, ByVal MsgTextBody As String, ByVal MsgHTMLBody As String, ByVal SenderName As String, ByVal SenderAddress As String, Optional ByVal MsgCharset As String = Nothing, Optional ByRef bufErrorDetails As String = Nothing, Optional ByVal Attachments As CompuMaster.camm.WebManager.Messaging.EMailAttachment() = Nothing, Optional ByVal Priority As CompuMaster.camm.WebManager.Messaging.EMails.Priority = Nothing, Optional ByVal Sensitivity As CompuMaster.camm.WebManager.Messaging.EMails.Sensitivity = Nothing, Optional ByVal RequestTransmissionConfirmation As Boolean = False, Optional ByVal RequestReadingConfirmation As Boolean = False, Optional ByVal AdditionalHeaders As Collections.Specialized.NameValueCollection = Nothing) As Boolean
            ...
            Select Case _WebManager.SMTPAuthType
                ...
                Case "CRAM-MD4"
                    Throw New NotSupportedException("CRAM-MD4 is not supported")
                Case "NTLM"
                    Throw New NotImplementedException("NTLM is not implemented yet")
                Case Else
                    'SmtpAuthMethod = "NONE"
            End Select
            ...
            If Not Attachments Is Nothing Then
                For MyCounter As Integer = 0 To Attachments.Length - 1
                    ...
                    If Attachments(MyCounter).PlaceholderInMhtmlToBeReplacedByContentID <> Nothing Then
                        Try
                            'Look at the workflow @ EasyMail how embedded attachments are supported
                            'Supported @ .Net 2.0
                            'System.Environment.StackTrace doesn't work with medium-trust --> work around it using a new exception class
                            Dim WorkaroundEx As New Exception("")
                            Dim WorkaroundStackTrace As String = WorkaroundEx.StackTrace 'contains only last few lines of stacktrace
            ...
            Try
                With cmd
                    .CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; " & vbNewLine & _
                                    "SELECT [Data] FROM [dbo].[Log_eMailMessages] where [ID] = " & queuedItemID
                    .CommandType = CommandType.Text
                    .Connection = dbConn
