...
Namespace CompuMaster.camm.SmartWebEditor
    ...
    Namespace Controls
        ...
        ''' </history>
        <Obsolete("Use SmartWcms3 or CompuMaster.camm.WebEditor4 instead")> Public Class SmartWcms
            Inherits CompuMaster.camm.WebManager.Controls.Control
            Implements UI.INamingContainer, ISmartWcmsEditor, IEditControl

            Private Interface IEditControl
                ReadOnly Property EditModeActive() As Boolean
            End Interface
...
#Const DebugMode = False
            ...
            ''' -----------------------------------------------------------------------------
            ''' <summary>
            '''     The internal web editor instance
            ''' </summary>
            ''' <value></value>
            ''' <remarks>
            ''' </remarks>
            ''' <history>
            ''' 	[adminsupport]	19.04.2006	Created
            ''' </history>
            ''' -----------------------------------------------------------------------------
            Public ReadOnly Property RadEditor() As Telerik.WebControls.RadEditor
                Get
                    Return editorMain
                End Get
            End Property
            ...
            Private Function FindSWcmsControlsInEditModeOnThisPage() As Boolean
                ...
                For MyCounter As Integer = 0 To EditorControls.Count - 1
                    If EditorControls(MyCounter) Is Me Then
                        'do nothing
                    ElseIf CType(EditorControls(MyCounter), IEditControl).EditModeActive = True Then
                        Result = True
                    End If
                Next
                Return Result
...
#End Region
            ...
            Protected editorMain As sWcmsRadEditor3
...
#Region "Page events"
            ...
            Private Sub PageOnInit(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Init
                ...
                'sWcms will not work correctly with the new Asp.Net 4 ClientID-Mode, because JavaScript registration fails.
                'clientIDMode AutoID has the same behaviour as .Net 1.1-3.5
                'TODO: CType(Page, Object).ClientIDMode works with the page directive <% @Page clientIDMode="AutoID" %> within an aspx file but currently not with the web.config entry <pages clientIDMode="AutoID" /> ==> CType(Page, Object).ClientIDMode is still '0' and not as required and exprected '1'
                If System.Environment.Version.Major >= 4 AndAlso CType(Page, Object).ClientIDMode <> 1 Then 'AutoID = 1; AutoID is required
                    Throw New Exception("ClientIDMode must be set to AutoID to allow the sWcmsRadEditor3 to work correctly (currently it's value is: " & CType(CType(Page, Object).ClientIDMode, Integer).ToString & "; AutoID would be: 1); please define it in web.config using <system.web><pages clientIDMode=""AutoID"" /></system.web> or assign the value to the <%@Page ClientIDMode=""AutoID"" %> directive.")
                End If
                ...
                For MyCounter As Integer = 0 To Me.Controls.Count - 1
                    ...
                    If compareControl Is editorMain OrElse
                            ...
                            compareControl Is ibtnSwitchToEditMode Then
                ...
                If Not ShowWithEditRights OrElse (ShowWithEditRights AndAlso Page.IsPostBack = False) Then
                    ...
                    Else
                        ...
                        Raise404("No file version available or unauthorized access")
                    End If
                End If
            End Sub
...
#End If
                ...
                If Me.ShowWithEditRights Then
                    ...
                    If Me.editorMain.Editable = True Then
                        ...
                        Me.HtmlString4VersionBrowsing = PrepareVersionHistory()
...
#Region " Control specific methods "
            ...
            ''' -----------------------------------------------------------------------------
            ''' <summary>
            '''     Initializes the toolsfile for the editors toolbar
            ''' </summary>
            ''' <remarks>
            ''' </remarks>
            ''' <history>
            ...
            Private Sub InitializeToolbar()
                ...
                'Init of the toolsfile
                Dim ResourceName As String = Nothing

                If Me.MarketLookupMode = MarketLookupModes.SingleMarket Then
                    If ToolbarSetting = ToolbarSettings.EditEditableVersion Then
                        'editorMain.ToolsFile = "~/system/modules/smartwcms/config/SmartWcms_EditEditable_SingleMarket.xml"
                        ResourceName = "ToolsBar_EditEditable_SingleMarket"
                    ElseIf ToolbarSetting = ToolbarSettings.EditNoneEditableVersions Then
                        'editorMain.ToolsFile = "~/system/modules/smartwcms/config/SmartWcms_EditNoneEditable_SingleMarket.xml"
                        ResourceName = "ToolsBar_EditNoneEditable_SingleMarket"
                    ElseIf ToolbarSetting = ToolbarSettings.EditWithValidEditVersion Then
                        'editorMain.ToolsFile = "~/system/modules/smartwcms/config/SmartWcms_VersionBrowsing_SingleMarket.xml"
                        ResourceName = "ToolsBar_VersionBrowsing_SingleMarket"
                    ElseIf ToolbarSetting = ToolbarSettings.NoVersionAvailable Then
                        'editorMain.ToolsFile = "~/system/modules/smartwcms/config/SmartWcms_NoVersionAvailable.xml"
                        ResourceName = "ToolsBar_NoVersionAvailable"
                    End If
                Else
                    If ToolbarSetting = ToolbarSettings.EditEditableVersion Then
                        'editorMain.ToolsFile = "~/system/modules/smartwcms/config/SmartWcms_EditEditable.xml"
                        ResourceName = "ToolsBar_EditEditable"
                    ElseIf ToolbarSetting = ToolbarSettings.EditNoneEditableVersions Then
                        'editorMain.ToolsFile = "~/system/modules/smartwcms/config/SmartWcms_EditNoneEditable.xml"
                        ResourceName = "ToolsBar_EditNoneEditable"
                    ElseIf ToolbarSetting = ToolbarSettings.EditWithValidEditVersion Then
                        'editorMain.ToolsFile = "~/system/modules/smartwcms/config/SmartWcms_VersionBrowsing.xml"
                        ResourceName = "ToolsBar_VersionBrowsing"
                    ElseIf ToolbarSetting = ToolbarSettings.NoVersionAvailable Then
                        'editorMain.ToolsFile = "~/system/modules/smartwcms/config/SmartWcms_NoVersionAvailable.xml"
                        ResourceName = "ToolsBar_NoVersionAvailable"
                    End If
                ...
                Me.editorMain.LoadToolsXml(Utils.ResourceValue("WebEdit", ResourceName))

            End Sub

            ''' -----------------------------------------------------------------------------
            ''' <summary>
            '''     Returns all the files stored in the global image file folder
            ''' </summary>
            ''' <returns></returns>
            ''' <remarks>
            ''' </remarks>
            ''' <history>
            ''' 	[swiercz]	22.12.2005	Created
            ''' </history>
            ''' -----------------------------------------------------------------------------
            Private Function GetFilesInImageFolder() As System.Web.UI.WebControls.ListItem()
                Dim Path As String = HttpContext.Current.Server.MapPath("images")
                Dim Result As System.Web.UI.WebControls.ListItem()
                Dim Files As System.IO.FileInfo() = New System.IO.DirectoryInfo(Path).GetFiles()
                ReDim Result(Files.Length - 1)
                For i As Integer = 0 To Files.Length - 1
                    Dim LI As New System.Web.UI.WebControls.ListItem
                    LI.Text = Files(i).Name
                    LI.Value = "<img src=""images/" & Files(i).Name & """ />"
                    Result(i) = LI
                Next
                Return Result
            End Function 'GetFilesInImageFolder()

            ''' -----------------------------------------------------------------------------
            ''' <summary>
            '''     Forward all editor control related property values to the editor control
            ''' </summary>
            ''' <remarks>
            ''' </remarks>
            ''' <history>
            ''' 	[adminsupport]	05.05.2006	Created
            ''' </history>
            ''' -----------------------------------------------------------------------------
            Private Sub AssignPropertiesToChildEditor()
                If Me.Docs <> Nothing Then
                    Me.editorMain.UploadDocumentsPaths = New String() {Utils.FullyInterpretedVirtualPath(Me.Docs)}
                    Me.editorMain.DeleteDocumentsPaths = New String() {Utils.FullyInterpretedVirtualPath(Me.Docs)}
                End If
                If Me.Media <> Nothing Then
                    Me.editorMain.UploadMediaPaths = New String() {Utils.FullyInterpretedVirtualPath(Me.Media)}
                    Me.editorMain.UploadFlashPaths = New String() {Utils.FullyInterpretedVirtualPath(Me.Media)}
                    Me.editorMain.DeleteMediaPaths = New String() {Utils.FullyInterpretedVirtualPath(Me.Media)}
                    Me.editorMain.DeleteFlashPaths = New String() {Utils.FullyInterpretedVirtualPath(Me.Media)}
                End If
                If Me.Images <> Nothing Then
                    Me.editorMain.DeleteImagesPaths = New String() {Utils.FullyInterpretedVirtualPath(Me.Images)}
                End If
                Me.editorMain.UploadImagesPaths = New String() {}
                Me.editorMain.DocumentsFilters = Me.DocsUploadFilter
                Me.editorMain.DocumentsPaths = PathsWithoutDuplicateMeaning(DocsReadOnly, Nothing)
                Me.editorMain.MediaPaths = PathsWithoutDuplicateMeaning(MediaReadOnly, Nothing)
                Me.editorMain.FlashPaths = PathsWithoutDuplicateMeaning(MediaReadOnly, Nothing)
                Me.editorMain.ImagesPaths = PathsWithoutDuplicateMeaning(ImagesReadOnly, Nothing)
                Me.editorMain.MaxDocumentSize = Me.DocsUploadSizeMax
                Me.editorMain.MaxImageSize = Me.ImagesUploadSizeMax
                Me.editorMain.MaxFlashSize = Me.MediaUploadSizeMax
                Me.editorMain.MaxMediaSize = Me.MediaUploadSizeMax
            End Sub
            ...
            Protected Overrides Sub CreateChildControls()
                ...
                editorMain = New sWcmsRadEditor3
                editorMain.SaveInFile = False
                editorMain.Editable = True
            ...
            ''' </history>
            ''' -----------------------------------------------------------------------------
            Private Function PathsWithoutDuplicateMeaning(ByVal paths As String(), ByVal singlePath As String) As String()
            ...
            ''' </history>
            ''' -----------------------------------------------------------------------------
            Private Function EncryptAStringForUrlUsement(ByVal sourceString As String) As String
            ...
            ''' -----------------------------------------------------------------------------
            ''' <summary>
            '''     Registers several needed scripts
            ''' </summary>
            ''' <remarks>
            ''' </remarks>
            ''' <history>
            ...
            ''' </history>
            ''' -----------------------------------------------------------------------------
            Private Sub JavaScriptRegistration()
                Dim myStringBuilder As New System.Text.StringBuilder
                myStringBuilder.Append("<script language=""javascript"">" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("/* Activate a Version of a Page and deactivate all others              */" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("function ActivateAPageVersionAndDeactivateAllOthers(){" + vbCrLf)
                myStringBuilder.Append("	document.getElementById('" & Me.ClientID & "_txtActivate').innerText = 'activate';" + vbCrLf)
                myStringBuilder.Append("	document.forms['" & Me.LookupParentServerForm.ClientID & "'].submit();" + vbCrLf)
                myStringBuilder.Append("}" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("/* Deactivate all available versions of a document                     */" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("function DeactivateAllPageVersions(){" + vbCrLf)
                myStringBuilder.Append("	document.getElementById('" & Me.ClientID & "_txtActivate').innerText = 'deactivate';" + vbCrLf)
                myStringBuilder.Append("	document.forms['" & Me.LookupParentServerForm.ClientID & "'].submit();" + vbCrLf)
                myStringBuilder.Append("}" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("/* Update a pages content                                              */" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("function updateAPagesVersion(RadEditorContentArea){" + vbCrLf)
                myStringBuilder.Append("	document.getElementById('" & Me.ClientID & "_txtNewVersion').innerText = 'update';" + vbCrLf)
                myStringBuilder.Append("	document.forms['" & Me.LookupParentServerForm.ClientID & "'].submit();" + vbCrLf)
                myStringBuilder.Append("}" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("/* Drop a market's data                                                */" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("function dropMarketData(RadEditorContentArea){" + vbCrLf)
                myStringBuilder.Append("	document.getElementById('" & Me.ClientID & "_txtNewVersion').innerText = 'dropcurrentmarket';" + vbCrLf)
                myStringBuilder.Append("	document.forms['" & Me.LookupParentServerForm.ClientID & "'].submit();" + vbCrLf)
                myStringBuilder.Append("}" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("/* Create a new page version                                           */" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("function createANewPageVersion(RadEditorContentArea){" + vbCrLf)
                myStringBuilder.Append("	document.getElementById('" & Me.ClientID & "_txtNewVersion').innerText = 'newversion';" + vbCrLf)
                myStringBuilder.Append("	document.forms['" & Me.LookupParentServerForm.ClientID & "'].submit();" + vbCrLf)
                myStringBuilder.Append("}" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("/* Provides multilingual document versions                             */" + vbCrLf)
                myStringBuilder.Append("/* The currently selected depends on the given market                  */" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("function showDropdownChooseLanguage(RadEditorContentArea, RadEditorToolImage)" + vbCrLf)
                myStringBuilder.Append("{" + vbCrLf)
                myStringBuilder.Append("   var html = """)
                myStringBuilder.Append(PrepareAvailableLanguages())
                myStringBuilder.Append(""";" + vbCrLf)
                myStringBuilder.Append("   radEditorShowPopup(html , 300, 250, RadEditorToolImage);" + vbCrLf)
                myStringBuilder.Append("}" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("/* Prepare the versions browsing dropdowns content                     */" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("function showDropdownVersionBrowsing(RadEditorContentArea, RadEditorToolImage)" + vbCrLf)
                myStringBuilder.Append("{" + vbCrLf)
                myStringBuilder.Append("   var html = """)
                myStringBuilder.Append(Me.HtmlString4VersionBrowsing)
                myStringBuilder.Append(""";" + vbCrLf)
                myStringBuilder.Append("   radEditorShowPopup(html, 100, 200, RadEditorToolImage);" + vbCrLf)
                myStringBuilder.Append("}" + vbCrLf)
                myStringBuilder.Append("// This function is executed when the user clicks an item of the  dropdown." + vbCrLf)
                myStringBuilder.Append("function clickItem(name)" + vbCrLf)
                myStringBuilder.Append("{" + vbCrLf)
                myStringBuilder.Append("   var range = document.selection.createRange();" + vbCrLf)
                myStringBuilder.Append("   range.pasteHTML(name);" + vbCrLf)
                myStringBuilder.Append("   radEditorPopupHide();" + vbCrLf)
                myStringBuilder.Append("}" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("/* Function to open the picture upload form                            */" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("function openPictureUploadForm()" + vbCrLf)
                myStringBuilder.Append("{" + vbCrLf)
                myStringBuilder.Append("    window.open('/sysdata/modules/smartwcms/upload.aspx?ref=" & Me.EncryptAStringForUrlUsement(DocumentID) & "&imageupload=" & Me.EncryptAStringForUrlUsement(Utils.FullyInterpretedVirtualPath(Me.Images)) & "&securityobject=" & EncryptAStringForUrlUsement(Me.SecurityObjectEditMode) & "','UploadForm','location=no,hotkeys=no,toolbar=no,dependent=yes');")
                myStringBuilder.Append("}" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("/* Function to start the preview                                       */" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("function activatePreview()" + vbCrLf)
                myStringBuilder.Append("{" + vbCrLf)
                myStringBuilder.Append("	document.getElementById('" & Me.ClientID & "_txtEditMode').innerText = 'false';" + vbCrLf)
                myStringBuilder.Append("	document.forms['" & Me.LookupParentServerForm.ClientID & "'].submit();" + vbCrLf)
                myStringBuilder.Append("}" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("/* Function to paste some code into the current editor position        */" + vbCrLf)
                myStringBuilder.Append("/***********************************************************************/" + vbCrLf)
                myStringBuilder.Append("function pasteTextIntoEditor(text)" & vbCrLf)
                myStringBuilder.Append("{" & vbCrLf)
                myStringBuilder.Append("	//paste text in the active selection, that is - at the editor's cursor position" & vbCrLf)
                myStringBuilder.Append("	var editor = document.getElementById('" & editorMain.ClientID & "');" & vbCrLf)
                myStringBuilder.Append("	editor.focus();" & vbCrLf)
                myStringBuilder.Append("	var range = document.selection.createRange();" & vbCrLf)
                myStringBuilder.Append("	range.pasteHTML(text);" & vbCrLf)
                myStringBuilder.Append("}" & vbCrLf)
                myStringBuilder.Append("</script>" + vbCrLf)
                If Not Me.Page.IsStartupScriptRegistered("CWM.sWcms.Startup") Then
                    Me.Page.RegisterStartupScript("CWM.sWcms.Startup", myStringBuilder.ToString())
                End If
            End Sub    'JavaScriptRegistration()
...
#Region " Properties "
            ...
            ''' </history>
            ''' -----------------------------------------------------------------------------
            Public ReadOnly Property EditModeActive() As Boolean Implements IEditControl.EditModeActive
                Get
                    Return editorMain.Visible
                End Get
            End Property
...
#End Region
            ...
            Dim _HtmlString4VersionBrowsing As String
            ''' -----------------------------------------------------------------------------
            ''' <summary>
            '''     Needed to initialize the version browsing dropdowns content
            ''' </summary>
            ''' <value></value>
            ''' <remarks>
            ''' </remarks>
            ...
            ''' </history>
            ''' -----------------------------------------------------------------------------
            Public Property HtmlString4VersionBrowsing() As String
                Get
                    Return _HtmlString4VersionBrowsing
                End Get
                Set(ByVal Value As String)
                    _HtmlString4VersionBrowsing = Value
                End Set
            End Property
            ...
            Private Enum ToolbarSettings As Integer
                None = 0
            ...
            ''' </history>
            ''' -----------------------------------------------------------------------------
            Private Property ToolbarSetting() As ToolbarSettings
                Get
            ...
            ''' </history>
            ''' -----------------------------------------------------------------------------
            <System.ComponentModel.TypeConverter(GetType(StringArrayConverter))> Public Property DocsUploadFilter() As String()
                Get
            ...
            ''' </history>
            ''' -----------------------------------------------------------------------------
            <System.ComponentModel.TypeConverter(GetType(StringArrayConverter))> Public Overloads Property DocsReadOnly() As String()
                Get
            ...
            ''' </history>
            ''' -----------------------------------------------------------------------------
            <System.ComponentModel.TypeConverter(GetType(StringArrayConverter))> Public Overloads Property MediaReadOnly() As String()
                Get
            ...
            ''' </history>
            ''' -----------------------------------------------------------------------------
            <System.ComponentModel.TypeConverter(GetType(StringArrayConverter))> Public Overloads Property ImagesReadOnly() As String()
                Get
...
#Region " Anything else "
            ...
            ''' -----------------------------------------------------------------------------
            ''' <summary>
            '''     Provides the HTML links needed for the version history
            ''' </summary>
            ''' <returns></returns>
            ''' <remarks>
            ''' </remarks>
            ...
            ''' </history>
            ''' -----------------------------------------------------------------------------
            Private Function PrepareVersionHistory() As String
                Dim result As String = Nothing
                Dim myActiveVersion As Integer = Me.Database.ReleasedVersion(Me.ContentOfServerID, Me.DocumentID, Me.EditorID)
                Dim MaxVersion As Integer = Me.Database.MaxVersion(Me.ContentOfServerID, Me.DocumentID, Me.EditorID)
                result &= "<table style='border:1px solid black' width='100%' height='100%'>"
                result &= "   <tr>"
                result &= "      <td valign='top'>"
                Dim myIndex As Integer = System.Math.Max(MaxVersion - 7, 1)
                Dim myDataTable As DataTable
                myDataTable = Database.ReadAllData(Me.ContentOfServerID, DocumentID, Me.EditorID)
                For i As Integer = myIndex To MaxVersion
                    Dim myDate As Date
                    For Each myDataRow As DataRow In myDataTable.Rows
                        If CType(myDataRow("Version"), Integer) = i Then
                            myDate = Utils.Nz(myDataRow("ReleasedOn"), CType(Nothing, Date))
                            Exit For
                        End If
                    Next
                    ...
                    End If
                    If i = myActiveVersion Then
                        If Me.MarketLookupMode = MarketLookupModes.SingleMarket Then
                            result &= "<div style=\""margin:2px;cursor:default;border:1px solid gray;font:icon\"" onclick=\""parent.document.getElementById('" & Me.ClientID & "_txtRedirUrl').innerText =  '" & i & ";0';parent.document.forms['" & Me.LookupParentServerForm.ClientID & "'].submit();\"" unselectable=\""on\""><b><font color=\""green\"">" & i & " " & ReleaseDate & "</font></b></div>"
                        Else
                            result &= "<div style=\""margin:2px;cursor:default;border:1px solid gray;font:icon\"" onclick=\""parent.document.getElementById('" & Me.ClientID & "_txtRedirUrl').innerText =  '" & i & ";" & cammWebManager.UI.MarketID & "';parent.document.forms['" & Me.LookupParentServerForm.ClientID & "'].submit();\"" unselectable=\""on\""><b><font color=\""green\"">" & i & " " & ReleaseDate & "</font></b></div>"
                        End If
                    Else
                        If Me.MarketLookupMode = MarketLookupModes.SingleMarket Then
                            result &= "<div style=\""margin:2px;cursor:default;border:1px solid gray;font:icon\"" onclick=\""parent.document.getElementById('" & Me.ClientID & "_txtRedirUrl').innerText =  '" & i & ";0';parent.document.forms['" & Me.LookupParentServerForm.ClientID & "'].submit();\"" unselectable=\""on\""><b><font color=\""red\"">" & i & " " & ReleaseDate & "</font></b></div>"
                        Else
                            result &= "<div style=\""margin:2px;cursor:default;border:1px solid gray;font:icon\"" onclick=\""parent.document.getElementById('" & Me.ClientID & "_txtRedirUrl').innerText =  '" & i & ";" & cammWebManager.UI.MarketID & "';parent.document.forms['" & Me.LookupParentServerForm.ClientID & "'].submit();\"" unselectable=\""on\""><b><font color=\""red\"">" & i & " " & ReleaseDate & "</font></b></div>"
                        End If
                    End If
                Next
                result &= "      </td>"
                ...
                result &= "</table>"
                Return result
            End Function 'PrepareVersionHistory()
